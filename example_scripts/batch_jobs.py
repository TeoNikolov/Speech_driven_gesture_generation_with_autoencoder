...
DATASET = os.path.join(SCRIPT_DIR, "..", "..", "..", "dataset", "full")
...
if args.train_gg:
    for gdim in GG_DIMS:
        for adim in AE_DIMS:
            subprocess.run(f'python {PIPELINE_SCRIPT} --dataset {DATASET} -adim {adim} -aeps {AE_EPOCHS} -gdim {gdim} -geps {GG_EPOCHS} -period {GG_PERIOD} -gg', shell=True)
    ...
    for gdim in GG_DIMS:
        for adim in AE_DIMS:
            ...
            for in_file in glob.glob(os.path.join(PREDICT_DIR_IN, "*.wav")):
                ...
                subprocess.run(f'python {PIPELINE_SCRIPT} --dataset {DATASET} --predict_in {p_in} --predict_out {p_out} -adim {adim} -gdim {gdim} -pred', shell=True)
